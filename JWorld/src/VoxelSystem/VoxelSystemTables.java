...
public final class VoxelSystemTables {
	 ...
	 * on that edge.
	 */
	public static int getEdgeFromMaterials(VoxelCube val) {
		int edgeInfo = 0;
		
		int t0 = val.getType(0);
		int t1 = val.getType(1);
		int t2 = val.getType(2);
		int t3 = val.getType(3);
		int t4 = val.getType(4);
		int t5 = val.getType(5);
		int t6 = val.getType(6);
		int t7 = val.getType(7);
	...
	/**
	 * intersection % from v1 to v2
	 * @param valp1
	 * @param valp2
	 * @return
	 */
	public static float getIntersection(float valp1,float valp2) {
		if (Math.abs(valp1) == 0)
			return 0f;
		if (Math.abs(valp2) == 0)
			return 1f;
